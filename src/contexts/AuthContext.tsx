...
/**
 * useUser - hook مبسط للوصول للمستخدم
 * 
 * حل جذري لمشكلة DEMO_USER_ID المنتشرة في 9 ملفات
 * يوفر:
 * - userId: المعرف الفريد للمستخدم (أو null)
 * - isAuthenticated: هل سجل الدخول
 * - isLoading: هل يتم التحميل
 * - requireAuth: يطلب تسجيل الدخول إذا لم يكن مسجل
 * 
 * @example
 * const { userId, isAuthenticated } = useUser();
 * // بدلاً من: const DEMO_USER_ID = 'demo-user';
 */
export function useUser() {
  const { user, isLoading, isAuthenticated } = useAuth();
  
  return {
    /** معرف المستخدم أو null إذا لم يسجل الدخول */
    userId: user?.id ?? null,
    /** كائن المستخدم الكامل */
    user,
    /** هل يتم تحميل بيانات المستخدم */
    isLoading,
    /** هل سجل المستخدم الدخول */
    isAuthenticated,
    /** يطلب من المستخدم تسجيل الدخول إذا لم يكن مسجل */
    requireAuth: () => {
