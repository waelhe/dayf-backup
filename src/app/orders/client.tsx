...
export default function OrdersClient() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      const response = await fetch(`/api/orders?userId=${effectiveUserId}`);
      if (response.ok) {
        const data = await response.json();
        setOrders(data.orders || []);
      }
    } catch (error) {
      logger.error('Error fetching orders', { error: error instanceof Error ? error.message : 'Unknown error' });
    } finally {
      setLoading(false);
    }
  };
