...
// Demo user ID - TODO: استبدال بـ useUser بعد إصلاح Auth API
...
export default function MarketplaceClient() {
  ...
  const fetchCart = async () => {
    try {
      ...
      const data = await response.json();
      if (data.items) {
        setCartItems(data.items.map((item: { product: Product; quantity: number }) => ({
          ...item.product,
      ...
      logger.error('Error fetching cart', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
  };
  ...
  const handleAddToCart = async (product: Product) => {
    ...
    try {
      const response = await fetch('/api/cart', {
        ...
        body: JSON.stringify({ userId: effectiveUserId, productId: product.id, quantity: 1 }),
      });
  ...
  const updateQuantity = async (productId: string, delta: number) => {
    ...
    try {
      await fetch('/api/cart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ userId: effectiveUserId, productId, quantity: newQuantity }),
      });
    } catch (error) {
      logger.error('Error updating quantity', { error: error instanceof Error ? error.message : 'Unknown error' });
    }
  };
