...
export function BookingModal({
  ...
  const { userId, requireAuth } = useUser();
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(language === 'ar' ? 'تم إنشاء الحجز بنجاح!' : 'Booking created successfully!');
        onClose();
      ...
      logger.error('Booking error', { error: error instanceof Error ? error.message : 'Unknown error' });
      toast.error(error instanceof Error ? error.message : (language === 'ar' ? 'فشل في إنشاء الحجز' : 'Failed to create booking'));
    } finally {
