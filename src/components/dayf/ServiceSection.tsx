...
export default function ServiceSection({ 
  ...
  const fetchServices = async () => {
    try {
      ...
      logger.info('Fetched services', { count: data.services?.length || 0, category });
      setServices(data.services || []);
    } catch (error) {
      logger.error('Error fetching services', { error: error instanceof Error ? error.message : 'Unknown error' });
      setServices([]);
    } finally {
      setLoading(false);
    }
