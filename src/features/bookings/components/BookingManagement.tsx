...
export default function BookingManagement({ providerId }: BookingManagementProps) {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Error fetching provider bookings', { error: err instanceof Error ? err.message : 'Unknown error' });
  ...
  const handleUpdateStatus = async (bookingId: string, newStatus: 'CONFIRMED' | 'CANCELLED') => {
    try {
      ...
      logger.error('Error updating booking status', { error: err instanceof Error ? err.message : 'Unknown error' });
